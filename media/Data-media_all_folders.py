...
def get_matched_areas(block: str):
    """
    对任意来源文件夹中的单篇文章做海域分类。
    使用统一的正向关键词表。
    """
    ...
    for area_code, area_re in AREA_RE.items():
...
def process_all_region_file(txt_path, info, source_folder):
    """
    任意来源文件夹中的文件：
    从原 TXT 中筛出命中各海域正向关键词的文章，
    并写入 OUT_DIR / 来源文件夹 / 海域代码 / 下。
    """
    ...
    for item in articles:
        ...
        matched_areas, matched_keywords_by_area = get_matched_areas(block)
    ...
    for area_code, kept_articles in area_articles.items():
        ...
        dst_path = OUT_DIR / source_folder / area_code / out_name
...
print("示例：sd/forum_sd-2016-01.txt -> classified_filtered_txt_by_region/sd/pr/forum_pr-2016-01.txt")
...
for txt_path, info in tqdm(target_files, desc="筛选归类 TXT", unit="file"):
    ...
    try:
        # 来源文件夹名，例如：/Downloads/sd/forum_sd-2016-01.txt -> sd
        try:
            source_folder = txt_path.relative_to(ROOT_DIR).parts[0].lower()
        except (ValueError, IndexError):
            source_folder = original_region

        result = process_all_region_file(
            txt_path=txt_path,
            info=info,
        ...
        if result["写出记录"]:
            ...
            records.append({
                ...
                "年份": year,
                "序号": seq,
                "目标地区/海域": f"{source_folder}_no_match",
                "总文章数": result["总文章数"],
                "写出文章数": 0,
                "政治梗排除文章数": result["政治梗排除文章数"],
                "未命中海域文章数": result["未命中海域文章数"],
