...
# 0. 精度控制参数
# ======================
# 5 位小数约为 1 米级，网页展示基本不明显变形
COORD_DECIMALS = 2

# 轻微拓扑保持简化，单位是度；0.00003° 约 3 米左右
# 如果导出的 HTML 仍然太大，可以改成 0.00005
SIMPLIFY_TOLERANCE = 0.01

GRID_SIZE = 10 ** (-COORD_DECIMALS)
...
def reduce_geometry_precision(geom):
    ...
    if set_precision is not None:
        try:
            geom = set_precision(geom, GRID_SIZE)
        except Exception:
            pass
...
basins = basins.cx[111.0:114.5, 21.0:24.5]

# 降低 GIS 精度，减小 HTML 体积
basins["geometry"] = basins["geometry"].apply(reduce_geometry_precision)
basins = basins[
    basins.geometry.notnull() & (~basins.geometry.is_empty)
...
for _, row in basins.iterrows():
    ...
    basin_points[str(bid)] = {
        "lat": round(float(p.y), COORD_DECIMALS),
        "lng": round(float(p.x), COORD_DECIMALS)
    }
...
def make_arrow_head(lat1, lng1, lat2, lng2):
    ...
    return [
        [round(left_lat, COORD_DECIMALS), round(left_lng, COORD_DECIMALS)],
        [round(tip_lat, COORD_DECIMALS), round(tip_lng, COORD_DECIMALS)],
        [round(right_lat, COORD_DECIMALS), round(right_lng, COORD_DECIMALS)]
    ]
...
for bid_str in basin_data.keys():
    ...
    for sid in active_set:
        ...
        edges.append({
            "line": [
                [round(lat1, COORD_DECIMALS), round(lng1, COORD_DECIMALS)],
                [round(lat2, COORD_DECIMALS), round(lng2, COORD_DECIMALS)]
            ],
            "head": arrow_head
        })
...
for _, row in basins.iterrows():
    ...
    <div style="min-width:280px;">
        <b>Basin_ID:</b> {bid}<br>
        <b>Down_ID:</b> {int(row["Down_ID"])}<br>
...
js_code = """
<script>
var basinData = """ + json.dumps(basin_data, ensure_ascii=False, separators=(",", ":")) + """;
var basinDownChains = """ + json.dumps(basin_down_chains, ensure_ascii=False, separators=(",", ":")) + """;
var basinUpChains = """ + json.dumps(basin_up_chains, ensure_ascii=False, separators=(",", ":")) + """;
var flowEdgesByBasin = """ + json.dumps(flow_edges_by_basin, ensure_ascii=False, separators=(",", ":")) + """;
var basinTypeMap = """ + json.dumps(basin_type_map, ensure_ascii=False, separators=(",", ":")) + """;
